...
const TASK_CARDS = [
  ...
  },
  {
    ...
    dateClass: 'soon', dateIcon: 'event_upcoming', dateText: 'Due in 3 days',
  },
  {
    ...
    dateClass: 'soon', dateIcon: 'event_upcoming', dateText: 'Tomorrow',
  },
]
...
function Lifecycle({ data }: { data: NonNullable<TaskMetrics['lifecycle']> }) {
  const kindIcon: Record<string, string> = { base: 'radio_button_checked', ai: 'auto_awesome' }
        ...
        return (
          <li key={i} className="tp-life-item">
            <span className="tp-life-item-ic" style={{ color: kindColor[kind] }}>
              <span className="material-icons">{kindIcon[kind] ?? 'radio_button_checked'}</span>
            </span>
...
function TaskDetailsPopover({
  ...
  // Deadline derived from task on every render — stable because task only
  // changes when user clicks a different card.
  const deadlineRef = useRef(0)
  if (task) {
    const computed = Date.now() + task.metrics.time_to_deadzone.mins * 60000
    // Only update when the task changes (title-keyed)
    if (deadlineRef.current === 0 || deadlineRef.current < Date.now() - 1000) {
      deadlineRef.current = computed
    }
  }
        ...
        >
          ...
          <div className="tp-body">
            ...
            <div className="tp-footer">
              <button className="tp-foot-btn" style={{ '--fbtn': '#F97316' } as React.CSSProperties} onClick={() => act('Reschedule')}>
                <span className="material-icons">event_repeat</span>Reschedule
              </button>
              <button className="tp-foot-btn" style={{ '--fbtn': '#3B82F6' } as React.CSSProperties} onClick={() => act('Reassign')}>
                <span className="material-icons">person_add</span>Reassign
              </button>
              <button className="tp-foot-btn primary" style={{ '--fbtn': '#10B981' } as React.CSSProperties} onClick={() => act('Open deal')}>
        ...
        <div className="column">
          ...
          <div className="col-head">
            ...
            <div className="col-actions">
              <button className="col-btn"><span className="material-icons">add</span></button>
              <button className="col-btn"><span className="material-icons">more_horiz</span></button>
            </div>
          </div>
