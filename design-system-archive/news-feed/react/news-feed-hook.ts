...
export function mountCard(el: CardElement): (() => void) | void {
  ...
  // direction mirrors the card pair
  const dir = tone === 'bull' ? -38 : 38
  const rot = tone === 'bull' ? -4 : 4
  ...
  function buildBar() {
    barEl.innerHTML = feed
      .map((_, i) => `<div class="nf-seg${i === idx ? ' on' : ''}" style="--i:${i}"></div>`)
      .join('')
  }
  ...
  function paintBar() {
    ...
    for (let i = 0; i < segs.length; i++) {
      segs[i].classList.toggle('on', i === idx)
    }
  }
  ...
  function next() {
    if (busy) return
    busy = true
    const swapEls = [titleEl, dateEl]
    swapEls.forEach((n) => {
      n.style.setProperty('--swap-dir', `${dir}px`)
      n.style.setProperty('--swap-rot', `${rot}deg`)
