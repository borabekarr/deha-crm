...
function tweenValue(card: HTMLElement, from: number, d: SgCardData): void {
  const to = d.value
  const el = qs<HTMLElement>('[data-value]', card)
  if (!el) return
  ...
  function step(now: number) {
    ...
    p = 1 - Math.pow(1 - p, 3)
    el.textContent = fmtMoney(from + (to - from) * p, d.unit)
    if (p < 1) raf = requestAnimationFrame(step)
  }
  raf = requestAnimationFrame(step)
  setTimeout(() => {
    cancelAnimationFrame(raf)
    el.textContent = fmtMoney(to, d.unit)
  }, 810)
}
 ...
 */
export function sgcCardRef(el: HTMLElement | null, d: SgCardData): void {
  if (!el) return
  ...
  function toggleFilters(): void { el.classList.contains('filters-open') ? closeFilters() : openFilters() }
...
export function sgcCleanup(el: HTMLElement | null): void {
  if (!el) return
  const e = el as HTMLElement & { __sgcCleanup?: CardCleanup }
  e.__sgcCleanup?.()
