...
test.beforeEach(async ({ page }) => {
  ...
  await page.evaluate(() => document.querySelector('#date-picker')?.scrollIntoView({ block: 'center' }))
  await page.waitForTimeout(300)
})

test('interaction', async ({ page }) => {
  const section = page.locator('#date-picker')
...
test('visual snapshot', async ({ page }) => {
  await expect(page.locator('#date-picker')).toHaveScreenshot('date-picker-default.png')
})
