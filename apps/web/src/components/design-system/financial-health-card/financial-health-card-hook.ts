 ...
 *
 * React assigns refs bottom-up (children before parents), so by the time this
 * callback fires, all child refs (num, pin, pinNum, zones) are already set and
 * the JSX-rendered zone <span> elements are present in zones.children.
 * No queueMicrotask or innerHTML needed — children are resolved synchronously.
 */
export function fhcCardMountRef(
  ...
  // Zones are rendered as JSX <span> children — resolve them directly from the
  // DOM element so we are guaranteed non-null at mount time.
  const resolvedRefs: FhcRefs = {
    ...refs,
    // If a ref was somehow not wired, fall back to querySelector on the card el
    num:    refs.num    ?? (el.querySelector<HTMLDivElement>('.fhc-num')     ?? null),
    pin:    refs.pin    ?? (el.querySelector<HTMLDivElement>('.fhc-pin')     ?? null),
    pinNum: refs.pinNum ?? (el.querySelector<HTMLDivElement>('.fhc-pin-badge') ?? null),
    zones:  refs.zones  ?? (el.querySelector<HTMLDivElement>('.fhc-zones')   ?? null),
  }
