...
function RightPanel({
  ...
  const discussInRef = useCallback((el: HTMLDivElement | null) => {
    ...
    const log = el.querySelector<HTMLDivElement>('[data-discuss-log]')
    if (!fi || !fs || !log) return
        ...
        <div className="pc-discuss-in" ref={discussInRef}>
          ...
          <div
            ...
            onClick={(e) => {
              ...
              e.stopPropagation()
              if ((e.target as Element).closest('[data-discuss-send]')) return
              inputRef.current?.focus()
            }}
          >
            <input
...
function DetailOverlay({ d, open, onClose, onClosed, onApply, onToast, onRemoveCard }: DetailOverlayProps) {
...
export default function PipelineCard() {
  ...
  function openDetail(card: CardData) {
    setDetailCard(card)
    setDetailOpen(true)
  }
