...
//
// Called as ref={(el) => { streakCardRef(el); return () => cleanupStreakCard(el) }}
// The callback resolves the bar element from inside the card via querySelector
// so it is always non-null at animation start regardless of ref attachment order.
...
export function streakCardRef(
  ...
  // Resolve bar element from inside the card — eliminates the null-abort risk
  // that existed when bar was passed as a parameter (barRef.current could be null
  // when the card ref attached first, causing the entrance animation to abort).
  const barElOrNull = el.querySelector<HTMLDivElement>('.sc-bar-fill')
  if (!barElOrNull) return

  // Assign to non-null locals so TypeScript can see them as narrowed inside closures.
  const card: HTMLDivElement = el
  const barEl: HTMLDivElement = barElOrNull
  ...
  function runLoad() {
    ...
    barRevealTimer = setTimeout(() => {
      ...
      setBarWidth(barEl, 68.25, true)   // 6825/10000 = 68.25% (single source of truth)
      drawRing(card)
    }, 540)
    ...
    settleTimer = setTimeout(() => {
      ...
      const rp = card.querySelector<SVGCircleElement>('.ring-prog')
      if (rp) {
