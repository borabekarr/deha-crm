/**
 * buttons-proximity-hook.ts — DOM-side behavior for the ButtonsProximity pilot page.
 *
 * Copied from buttons/buttons-hook.ts (byte-for-byte logic, renamed header only):
 * the Apply-button state machine is unrelated to the proximity pilot and is kept
 * identical so the pilot page renders the same specimen behavior as the original.
 *
 * NO raw useEffect anywhere in the buttons-proximity/ folder.
