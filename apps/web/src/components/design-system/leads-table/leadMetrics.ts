/* =========================================================================
   leadMetrics.ts -- Lead Details Popover schema + tiered metric engine.
   Ported from apps/web/design-system/preview/_lead-metrics.js.
...
import { fmtTL } from './leadsData'

// ── Tone token refs (resolve inside .ldx-overlay) ─────────────────────────────

const T = {
  g:'var(--g)', gl:'var(--g-l)', a:'var(--a)', al:'var(--a-l)',
  r:'var(--r)', rl:'var(--r-l)', b:'var(--b)', bl:'var(--b-l)',
  v:'var(--v)', vl:'var(--v-l)', o:'var(--o)', ol:'var(--o-l)',
  txt:'var(--txt)',
}
const TBG: Record<string, [string, string, string]> = {
...
export const SCHEMA: SchemaItem[] = [
  // PILL 1 - Pulse (tier 0): verdict -> action -> analysis -> friction -> risk
...
function seeded(id: number, salt: number) {
  let t = ((id * 2654435761 + salt * 40503) >>> 0)
...
function toneFor(pct: number, invert: boolean): string {
  ...
  return p >= 67 ? 'g' : p >= 40 ? 'a' : 'r'
}

export const isActive = (l: Lead) => l.last < 3 * 1440
export const hasVoice = (l: Lead) => ![9,14,16].includes(l.id) && l.acts && l.acts.some(a => a.ic === 'call')

export function coldDropHours(l: Lead): number | null {
  if (l.temp === 'cold') return null
  const base = l.temp === 'hot' ? 30 : 16
  const r = seeded(l.id, 7)()
  return Math.round(base + r * 30 - (l.last / 1440) * 1.5)
}

function channelOf(l: Lead): string {
...
function cmp(delta: number, label: string) {
  return { pct: Math.abs(Math.round(delta)), dir: delta >= 0 ? 'up' : 'down', label: label || 'vs sector avg' }
}

const SPEED: Record<string, { lb: string; tone: string }> = {
  extreme: { lb:'Extreme speed', tone:'g' }, high: { lb:'High speed', tone:'b' },
  medium:  { lb:'Medium speed', tone:'a'  }, low:  { lb:'Low speed', tone:'slate' },
}

function channelsFor(l: Lead) {
  const r = seeded(l.id, 11)
  const pref = channelOf(l)
  const base = [
    { name:'WhatsApp',  icon:'chat',         t: Math.round(3 + r() * 9) },
    { name:'Phone',     icon:'call',         t: Math.round(45 + r() * 120) },
    { name:'Email',     icon:'mail',         t: Math.round(180 + r() * 360) },
    { name:'Instagram', icon:'photo_camera', t: Math.round(240 + r() * 900) },
  ]
  const pull = base.map(c => (c.name === pref ? 2.4 : 1) * (1 / Math.sqrt(c.t)) * (0.8 + r() * 0.5))
  const sum = pull.reduce((a, b) => a + b, 0)
  const sectAvg: Record<string, number> = { WhatsApp:34, Phone:140, Email:520, Instagram:900 }
  return base.map((c, i) => {
    ...
    const faster = Math.round((avg - c.t) / avg * 100)
...
function heatDays(l: Lead): number[] {
  ...
  for (let wk = 0; wk < W; wk++) {
    const f = wk / (W - 1) * 3, lo = Math.floor(f), hi = Math.min(3, lo + 1), t2 = f - lo
    const intensity = (a[lo] * (1 - t2) + a[hi] * t2) / max
    for (let d = 0; d < 7; d++) {
...
// Workspace-specific metric overrides are filtered by the caller via SCHEMA.ws,
// so this builder derives the full metric set from the lead alone.
export function buildLeadMetrics(l: Lead): MetricData {
  ...
  const bar   = (pct: number, tone: string, vlabel: string | null, note?: string) => ({ viz:'bar', pct:clamp(Math.round(pct),2,100), tone:(T as Record<string,string>)[tone+'l']||(T as Record<string,string>)[tone], vlabel, note })
  const split = (parts: unknown[], note?: string) => ({ viz:'split', parts, note })
  const level = (lv: number, labels: string[], tone: string, note?: string) => ({ viz:'level', lv, labels, tone:(T as Record<string,string>)[tone+'l']||(T as Record<string,string>)[tone], note })
  const phrase= (rows: unknown[], note?: string) => ({ viz:'phrase', rows, note })
  const pillv = (v: string, icon: string, tone: string, note?: string) => ({ viz:'pill', v, icon, tone:(T as Record<string,string>)[tone], toneBg:(TBG[tone]||TBG.g)[1], toneBd:(TBG[tone]||TBG.g)[2], note })
  const ring  = (pct: number, tone: string, label: string, note?: string) => ({ viz:'ring', pct:clamp(Math.round(pct),0,100), tone:(T as Record<string,string>)[tone+'l']||(T as Record<string,string>)[tone], label, note })
  const playbook = (rows: unknown[], opts?: Record<string, unknown>) => Object.assign({ viz:'playbook', rows }, opts || {})
  const heatmap  = (days: number[], tag: string, dir: string, note?: string) => ({ viz:'heatmap', days, tag, dir, note })
  const daywindow= (cfg: Record<string, unknown>) => Object.assign({ viz:'daywindow' }, cfg)
  const channels = (items: unknown[], note?: string) => ({ viz:'channels', items, note })
  const colbars  = (items: unknown[], note?: string) => ({ viz:'colbars', items, note })
  const elasticity = (cfg: Record<string, unknown>) => Object.assign({ viz:'elasticity' }, cfg)

  // PILL 1 - Pulse (tier 0)
  m.pulse_summary = playbook(summaryBullets(l)) as Record<string, unknown>
  ;(m.pulse_summary as Record<string, unknown>).hero = true

  const negSig = (l.signals || []).find(s => s.type === 'neg') || (l.signals || []).find(s => s.type === 'warn')
  m.anomaly = negSig
    ? pillv(negSig.text, 'warning', negSig.type === 'neg' ? 'r' : 'a', 'Flagged automatically from the event log - worth a look before the next touch.')
    : { viz:'status', ok:true, text:'No friction detected', sub:'all process signals normal' }
  ;(m.anomaly as Record<string, unknown>).themeAcc = 'var(--r-l)'
  ...
  if (!fUp) (m.velocity_trend as Record<string, unknown>).themeAcc = 'var(--r-l)'

  m.channel_share = channels(channelsFor(l), 'Where the conversation actually happens - and how fast each replies.')
  ...
  m.contact_time = daywindow({ startH, endH, dayLb, rate:60 + Math.round(r2() * 15), altRate:38 + Math.round(r2() * 8), note:'Best opened-and-replied window from past touches - line up the next call here.' })

  const lossDrop = clamp(100 - score + gapD * 1.5, 6, 96)
  m.loss_pulse = bar(lossDrop, toneFor(lossDrop, true) === 'g' ? 'g' : (lossDrop > 60 ? 'r' : 'a'), lossDrop + '%',
    lossDrop > 55 ? 'Critical gap since last touch - re-engage today.' : 'Stable for now; keep the cadence.')
  ;(m.loss_pulse as Record<string, unknown>).themeAcc = 'var(--r-l)'

  // PILL 2 - Behavior (tier 1)
  const dwellS = Math.round(40 + eng * 4 + r3() * 200)
  m.dwell = stat(Math.floor(dwellS / 60) + 'm ' + String(dwellS % 60).padStart(2, '0') + 's', 'net time on sent offers & PDFs', 'Time actually spent reading what you sent - not just opens.')
  const sectDwellS = 180
  const dwellDiff = Math.round((dwellS - sectDwellS) / sectDwellS * 100)
  ;(m.dwell as Record<string, unknown>).sectorBadge = { pct: Math.abs(dwellDiff), dir: dwellDiff >= 0 ? 'up' : 'down' }

  const spike = (l.signals || []).some(s => /open|view|re-?read|spike/i.test(s.text))
  m.reengage = spike
    ? pillv('Opened an old link ' + (2 + Math.floor(r3() * 4)) + 'x in 2h', 'restart_alt', 'g', 'Sudden return after a quiet spell - intent is back.')
  ...
  m.micro_hotspot = pillv(pick(['Price Table - Extra Costs','Payment Schedule','Sea-view photos','Floor Plan - Bedrooms'], r4()) + ' (+' + (90 + Math.floor(r4() * 60)) + '% focus)', 'my_location', 'b', 'The single element they fixate on and keep returning to.')
  ...
  const locs = (l.profile && l.profile.locations || 'Center - Suburb').split(/,|->|\//).map((s: string) => s.trim()).filter(Boolean)
  ...
  ;(m.decision_power as Record<string, unknown>).compare = cmp(decP - 58, decP >= 58 ? 'more autonomous than sector' : 'less autonomous than sector')
  ...
  ;(m.funding as Record<string, unknown>).steps = [ /cash/i.test(fin)
  ...
  ;(m.ltv_segment as Record<string, unknown>).seg = seg[0]
  ;(m.ltv_segment as Record<string, unknown>).steps = [{ ic: ltv >= 2.5e6 ? 'star' : 'trending_up', t: ltv >= 2.5e6 ? 'High-LTV - protect this relationship; assign your strongest closer.' : 'Look for an upsell or referral path to lift lifetime value.' }]
  ...
  m.price_ceiling = elasticity({
    spoken: spokenVal, max: ceil, fmt: (v: number) => fmtTL(Math.round(v / 1e5) * 1e5), headroom,
  ...
  m.loss_risk = { viz:'horizon', note:'One engine, two horizons - switch to see each risk.', horizons:[
    { id:'ghost', label:'Immediate ghost', pct:Math.round(ghost), tone:(T as Record<string,string>)[ghost > 60 ? 'rl' : 'al'],
      sub: ghost > 60 ? "Don't email the offer - book a live call." : 'Low ghost risk; a message is fine.' },
    { id:'churn', label:'Competitor churn', pct:Math.round(churn), tone:(T as Record<string,string>)[churn > 55 ? 'rl' : 'al'],
  ...
  m.velocity_dev = stat((dev > 0 ? '+' : '') + dev + '%', dev > 0 ? 'faster than top closers' : 'slower than top closers', 'Elapsed time vs the top closed-won average.')
  ;(m.velocity_dev as Record<string, unknown>).tone = dev >= 0 ? T.g : T.r
...
export function summaryFor(l: Lead): string {
  const h = l.score >= 70 ? 'g' : l.score >= 45 ? 'a' : 'r'
  const first = l.name.split(' ')[0]
